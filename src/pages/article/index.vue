...
import { drop, getArticleList, put } from './api'
  ...
  [
    ...
    { headerName: '名称', field: 'name', value: '', cellRenderer: { setup: ({ params }) => () =>
      <RouterLink
        ...
        to={{ name: 'article-id', params: { id: params.data.id } }}
      >
        {params.value}
      </RouterLink>,
    } },
    ...
    { headerName: '状态', field: 'status', suppressSizeToFit: true, value: 'true', form: { type: 'switch' }, cellRenderer: { setup: ({ params }) => () =>
      <ElSwitch
        disabled={!hasPermission('/articles/[id]/put')}
    ...
    { headerName: '操作', field: 'actions', maxWidth: 68, unCheck: true, suppressMovable: true, lockPosition: 'right', pinned: 'right', cellRenderer: { setup: props => () =>
      <div className="flex justify-between">
        ...
        />
        <button v-permission="/articles/[id]/delete" className="i-fa6-solid:trash-can btn"
...
<template>
  <div layout>
    <VHeader>
      <el-button v-permission="'/articles/post'" class="!ml-auto" type="primary" @click="addHandler">
