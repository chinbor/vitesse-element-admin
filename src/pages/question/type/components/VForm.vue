...
import { type QuestionType, getQuestionType, post, put } from '../api'
...
let row = $ref<QuestionType>({})
id && getQuestionType(id).then((res) => {
  row = res.data
})
...
<template>
  <el-dialog v-model="show" :close-on-click-modal="false" custom-class="!w-2xl" :title="`${id ? '修改' : '添加'}角色`">
