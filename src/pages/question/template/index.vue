...
const { agGridBind, agGridOn, selectedList, getList, row } = useAgGrid<Template>(
  () => [
    ...
    { headerName: '标题', field: 'title', value: '', cellRenderer: { setup: ({ params }) => () =>
      <router-link class="text-primary hover:opacity-70" to={{ name: 'question-template-id', params: { id: params.data.id }, query: { headerTitle: params.value } }}>{params.value}</router-link>,
    } },
