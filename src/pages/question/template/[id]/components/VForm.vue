...
const { id, templateId, ...props } = defineProps<{
  id: string
  templateId: string
  show: boolean
}>()

let row = $ref<Question>({ templateId, status: 1, questionFlag: 0, required: 0 })
...
<template>
  <el-dialog v-model="show" :close-on-click-modal="false" custom-class="!w-2xl" draggable :title="`${id ? '修改' : '添加'}角色`">
    <el-form ref="formRef" label-width="auto" :model="row" @submit.prevent="submit">
      <el-form-item :rules="[{ message: '不能为空', required: true }]" prop="content" label="名称">
