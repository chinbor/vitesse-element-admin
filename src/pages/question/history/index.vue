...
const { agGridBind, agGridOn } = useAgGrid<History>(
  () => [
    ...
    { headerName: '标题', field: 'title', value: '', cellRenderer: { setup: ({ params }) => () =>
      <router-link class="text-primary hover:opacity-70" to={{ name: 'question-history-id', params: { id: params.data.id }, query: { headerTitle: params.value } }}>{params.value}</router-link>,
    } },
