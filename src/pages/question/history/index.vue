...
const { agGridBind, agGridOn } = useAgGrid<History>(
  () => [
    ...
    { headerName: '标题', field: 'title', value: '', cellRenderer: { setup: ({ params }) => () =>
      <a v-permission_disabled="historyId" className="text-primary hover:opacity-70 cursor-pointer" onClick={() => useRouter().push({ name: 'question-history-id', params: { id: params.data.id }, query: { headerTitle: params.value } })}>{params.value}</a>,
    } },
