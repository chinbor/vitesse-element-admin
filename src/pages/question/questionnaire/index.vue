...
const { agGridBind, agGridOn, selectedList, getList, row } = useAgGrid<Questionnaire>(
  () => [
    ...
    { headerName: '标题', field: 'title', value: '', cellRenderer: { setup: ({ params }) => () =>
      <a v-permission_disabled="templateId" className="text-primary hover:opacity-70 cursor-pointer" onClick={() => router.push({ name: 'question-questionnaire-id', params: { id: params.data.id }, query: { headerTitle: params.value } })}>{params.value}</a>,
    } },
